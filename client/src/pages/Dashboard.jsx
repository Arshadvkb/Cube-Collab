      ...
      <main className="flex-1 flex flex-col h-screen overflow-y-auto">
        <header className="bg-white dark:bg-gray-900 px-8 py-6 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">Here are your documents.</p>
          </div>
          <button 
        ...
        <section className="p-8">
          {documents && documents.length > 0 ? (
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
             {documents.map((doc) => (
               <div key={doc._id} className="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 hover:shadow-md transition-all flex flex-col h-full">
                 <div className="flex items-start justify-between mb-4">
                   <div className="p-3 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg">
                     <FileText size={24} />
                   </div>
                   <span className={`text-xs px-2 py-1 rounded-full font-medium ${doc.isPublic ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'}`}>
                     {doc.isPublic ? 'Public' : 'Private'}
                   </span>
                 </div>
                 <h3 className="text-lg font-semibold text-gray-800 dark:text-white line-clamp-1 mb-2">{doc.title}</h3>
            ...
            <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-gray-900 rounded-xl border border-dashed border-gray-300 dark:border-gray-700">
              ...
              <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-2">No documents found</h3>
              <p className="text-gray-500 dark:text-gray-400 mb-6 text-center max-w-md">You haven't created any documents yet. Start by creating a new document to organize your ideas.</p>
              <button 
                onClick={() => navigate('/document/new')}
                className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-lg font-medium transition-colors shadow-sm"
              >
