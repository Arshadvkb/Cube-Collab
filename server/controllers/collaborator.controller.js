...
const addCollaborator=async(req,res)=>{
    ...
    try {
        ...
        if(collaborator.isVerified===false){
            ...
            return res.status(400).json({success:false,msg:"User is not verified"})
        }
        ...
        const newCollaborator = new collaborationModel({
         ...
         return res.status(200).json({success:true,msg:"New collaborator added successfuly"})        
    } catch (error) {
        console.log("error in adding collaborator :"+error.message);
        return res.status(500).json({success:false,msg:"error in adding collaborator",error:error.message})        
    }
}
