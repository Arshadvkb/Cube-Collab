...
const viewDoc = async (req, res) => {
  try {
    ...
    const sharedDocs = await collaborationModel
      .find({ userId: user })
      .populate('documentId');
    ...
    return res
      .status(200)
      .json({ success: true, msg: 'Available Documents', documents: doc });
  } catch (error) {
...
const updateDoc = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ success: true, msg: 'The Document edited successfuly' });
  } catch (error) {
